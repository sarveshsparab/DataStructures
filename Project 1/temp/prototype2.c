...
statusCode fileRead(empRec **,char []);
...
main()
{
    ...
    {
        check1=fileRead(&compA,"companyA.txt");
        if(!check1)
        {
        ...
        check2=fileRead(&compB,"companyB.txt");
        if(!check2)
        {
...
/*empRec *(empRec *rec1,empRec *rec2)
{
    empRec *ans;
...
statusCode fileRead(empRec **head,char fileChoice[])
{
    FILE *fp;
    empRec *ptr,*last;
...
empRec *uniqueList(empRec *head)
{
    ...
    int tempHour;
    char tempPhone[15],tempName[50],tempProj[50],tempAddr[100];
    char check[1],path[200];
    statusCode check1;
    printf("\nDo you want to import data from a FILE..(y/n)");
    scanf("%s",check);
    if(strcmp(check,"y")==0)
    {
        printf("\nIs the file in the same directory as the program? (y/n)");
        scanf("%s",check);
        if(strcmp(check,"y")==0)
        {
            printf("\nEnter file name: \n");
